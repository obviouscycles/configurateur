  ...
  try {
    ...
    setTimeout(() => {
      ...
      if (actions && btnReset && !document.getElementById('shared-mode-bloc')) {
        ...
        bloc.style.cssText = 'background:#1B1C20;border:0.5px solid #333;padding:1rem;margin-bottom:.75rem;';
        bloc.innerHTML =
          '<div style="font-size:11px;color:#B08D57;text-transform:uppercase;letter-spacing:.06em;margin-bottom:.5rem;">Votre demande est en cours</div>' +
          ...
          '<button onclick="openContactDrawer()" style="width:100%;background:none;border:0.5px solid #B08D57;color:#B08D57;padding:9px;font-size:12px;font-weight:600;cursor:pointer;font-family:var(--font);margin-bottom:.5rem;"><i class="ti ti-message"></i> Poser une question</button>' +
      ...
      if (p11FinalBtns && !document.getElementById('p11-shared-mode-bloc')) {
        ...
        p11Bloc.style.cssText = 'background:#1B1C20;border:0.5px solid #333;padding:1rem;margin-bottom:.75rem;';
        p11Bloc.innerHTML =
          '<div style="font-size:11px;color:#B08D57;text-transform:uppercase;letter-spacing:.06em;margin-bottom:.5rem;">Votre demande est en cours</div>' +
          '<p style="font-size:12px;color:#aaa;line-height:1.5;margin-bottom:.75rem;">Nous vous contacterons sous 48h pour finaliser votre projet.</p>' +
          '<button onclick="openContactDrawer()" style="width:100%;background:none;border:0.5px solid #B08D57;color:#B08D57;padding:9px;font-size:12px;font-weight:600;cursor:pointer;font-family:var(--font);margin-bottom:.5rem;"><i class="ti ti-message"></i> Poser une question</button>' +
...
let selOptColorIdx = {};
// Tuning : tuningId -> option_id choisi. Absent de la map = "standard" (aucun
// surcoût). Couleur du tuning choisi : même mécanisme que selOptColorIdx, clé
// "tuningId_optionId" -> index couleur.
let selTuning = {};
let selTuningColorIdx = {};
...
function computeTotals(modelId, opts) {
  ...
  let weight = cadreOpt ? cadreOpt.weight : 0;
  activePostMeta().forEach(p => {
    const allOpts = ALL_OPTIONS[p.id] || [];
    const opt = allOpts.find(o => o.id === opts[p.id]);
    if (opt) { price += opt.price; weight += opt.weight; }
  });
  price += computeTuningDelta(opts);
  const modelStats = (typeof MODEL_STATS !== 'undefined') ? MODEL_STATS[modelId] : null;
  if (modelStats) weight += modelStats.poidsBase;
  return { price, weight };
}

// V6 — Tuning : une case n'est active que si son poste parent a, dans SA sélection
// actuelle, une option qui figure dans requiresOptionIn (ou toujours active si
// requiresOptionIn est vide — cas du Cadre, qui n'a rien à "choisir").
function isTuningActive(tp, opts) {
  if (!tp.requiresOptionIn) return true;
  const current = opts[tp.parentPost];
...
// rien choisi (le "standard" s'applique alors implicitement, écart nul).
function computeTuningDelta(opts) {
  let delta = 0;
  ...
  POST_META.forEach(p => {
    ...
    cards += '<div class="recap-card">' +
      '<div class="recap-card-header"><i class="ti ' + (postIcons[p.id]||'ti-point') + '"></i>' + p.name +
        (isModCard ? '<span style="display:inline-block;width:6px;height:6px;border-radius:50%;background:#B08D57;margin-left:5px;vertical-align:middle;"></span>' : '') +
      '</div>' +
...
function dtPresetBar(modelId) {
  ...
  return '<div class="preset-bar" onclick="event.stopPropagation()">' +
    '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;">' +
      '<div class="preset-label" style="margin-bottom:0;color:#B08D57;">3 suggestions pour démarrer</div>' +
      '<button onclick="toggleDtPresetInfo(\'' + infoPopupId + '\')" style="background:none;border:none;color:#B08D57;font-size:22px;cursor:pointer;padding:0;line-height:1;" title="En savoir plus"><i class="ti ti-info-circle"></i></button>' +
    '</div>' +
    '<div id="' + infoPopupId + '" style="display:none;font-size:12px;color:#aaa;background:#17181B;border:0.5px solid #333;padding:10px 12px;margin-bottom:10px;line-height:1.7;">' +
...
function dtToggleOOD() {
  ...
  if (!popup) {
    ...
    popup.innerHTML =
      ...
      '<a href="https://www.obviouscycles.com/velos-titane/velo-titane-sur-mesure/" target="_blank" style="display:inline-flex;align-items:center;gap:6px;font-size:12px;color:#211C12;text-decoration:none;font-weight:700;background:#B08D57;padding:10px 14px;width:100%;box-sizing:border-box;justify-content:center;">' +
    ...
    inner.innerHTML = '<p class="section-title" style="color:#EDEAE2;margin-bottom:1.5rem;">Mes configurations (' + configs.length + ')</p>' +
      configs.map((c, idx) => {
        ...
        const devisBadge = c.devisSent
          ? '<span title="Devis envoyé le ' + (c.date || '') + '" style="display:inline-flex;align-items:center;gap:4px;background:#2A2419;color:#B08D57;font-size:10px;font-weight:600;padding:2px 7px;border-radius:10px;margin-left:8px;white-space:nowrap;"><i class="ti ti-send" style="font-size:11px;"></i>Devis envoyé</span>'
          : '';
        return '<div style="display:flex;align-items:center;gap:1rem;padding:1rem;background:#1D1F23;border:0.5px solid ' + (c.devisSent ? '#B08D57' : '#333') + ';margin-bottom:8px;">' +
...
// V9 — Bandeau de télémétrie façon HUD (poids en direct, nombre de postes
// configurés, statut) et barres de caractère (rigidité/confort/aéro/polyvalence).
// Les notes viennent de MODEL_STATS — DONNÉES PROVISOIRES en attendant un onglet
...
// Rafraîchit uniquement les valeurs du HUD (poids/nombre/statut), sans reconstruire
// tout le panneau gauche (photo, description...) — appelé à chaque sélection de
...
function renderComponentDimField(key, label, options, refreshFn, defaultValue) {
  ...
  if (options.length === 1) {
    ...
    return `<div class="comp-dim-field" onclick="event.stopPropagation()" style="margin-top:.6rem;padding-top:.6rem;border-top:0.5px solid #1F2024;">
...
// ═══ V6 — TUNING ═══════════════════════════════════════════════════════════
// Case de personnalisation rattachée à un composant déjà choisi (chape oversize,
// collier de selle, jeu de direction...). Même format visuel que l'écran
// "Personnalisation" (bordure dorée si coché), avec photo + couleurs en plus.
...
function renderTuningBox(tp, renderFn) {
  ...
  const cardsHtml = visibleOpts.map(o => {
    ...
    const delta = standard ? o.price - standard.price : o.price;
    const priceLabel = delta === 0 ? 'Inclus' : (delta > 0 ? '+' : '') + delta.toLocaleString('fr-FR') + ' €';
    const colorKey = tp.tuningId + '_' + o.id;
    const activeColorIdx = selTuningColorIdx[colorKey] || 0;
    const hasColors = o.couleurs && o.couleurs.length;
    const imgId = 'tuning-img-' + tp.tuningId + '-' + o.id;
    const imgSrc = hasColors ? o.couleurs[activeColorIdx].photo : o.image;
  ...
  return '<div class="tuning-box">' +
    '<div class="tuning-box-label"><i class="ti ti-adjustments"></i> ' + tp.label + '</div>' +
    '<div class="tuning-cards">' + cardsHtml + '</div>' +
  '</div>';
}
...
function p11ShowGuideResultButtons() {
  ...
  footer.innerHTML =
    ...
    '<button onclick="p11ChoisirResultatsSheet()" style="flex:2;background:#B08D57;border:none;color:#211C12;padding:12px;font-size:13px;font-weight:700;cursor:pointer;font-family:inherit;border-radius:8px;">Choisir ces résultats →</button>';
}
        ...
        activePostMeta().map(p => {
          ...
          return '<div style="display:flex;justify-content:space-between;align-items:center;padding:10px 0;border-bottom:0.5px solid #1D1F23;">' +
            '<div style="font-size:13px;color:'+(isModified?'#ccc':'#666')+';display:flex;align-items:center;gap:8px;">' +
              '<i class="ti '+(icons[p.id]||'ti-point')+'" style="font-size:11px;color:#B08D57;"></i>' +
              p.name +
    ...
    '<div style="margin-top:1.5rem;padding:1rem;background:#1D1F23;border:0.5px solid #333;">' +
      ...
      ((!Object.keys(selSize).some(k => selSize[k])) ?
        ...
        (() => {
          ...
          return '<div style="font-size:13px;color:#EDEAE2;line-height:2;display:flex;flex-wrap:wrap;gap:8px 24px;">' + parts.join('') + '</div>';
        })() +
      '</div>') +
    '</div>' +
        ...
        '<div style="display:flex;flex-direction:column;gap:6px;">' +
          tuningList.map(t =>
            '<div style="display:flex;justify-content:space-between;align-items:center;font-size:13px;">' +
              '<span style="color:#999;">' + t.label + ' — <span style="color:#EDEAE2;">' + t.optionName + '</span></span>' +
              '<span style="color:#B08D57;font-weight:500;flex-shrink:0;margin-left:12px;">' + (t.delta > 0 ? '+' : '') + t.delta.toLocaleString('fr-FR') + ' €</span>' +
            '</div>'
          ).join('') +
        '</div>' +
      '</div>';
...
function dtRenderRecap() {
  ...
  const cadreRowHtml = '<div class="dtr-row"' + (cadreSurMesure ? ' style="color:#B08D57;"' : '') + '>' +
    '<span class="dtr-lbl"' + (cadreSurMesure ? ' style="color:#B08D57;"' : '') + '><i class="ti ti-frame" style="font-size:8px;margin-right:3px;"></i>Cadre</span>' +
    '<span class="dtr-val"' + (cadreSurMesure ? ' style="color:#B08D57;font-weight:600;"' : '') + '>' + cadreVal + '</span>' +
  '</div>';
...
function dtQuickSave() {
  ...
  if (btn) {
    ...
    btn.innerHTML = '<i class="ti ti-check" style="color:#B08D57;"></i> Sauvegardé';
...
function evoRender() {
  ...
  container.innerHTML = opts.map(opt => {
    ...
    return `<div style="background:#17181B;border:0.5px solid ${checked ? '#B08D57' : '#1F2024'};padding:.9rem 1rem;border-radius:8px;transition:border-color .15s;">
      <div style="display:flex;align-items:flex-start;gap:.65rem;${isInserts ? '' : 'cursor:pointer;'}" ${isInserts ? '' : `onclick="evoToggle('${opt.id}')"`}>
        ...
        <div style="flex:1;">
          <div style="display:flex;justify-content:space-between;align-items:baseline;gap:.5rem;">
            <span style="font-size:13px;font-weight:500;color:#EDEAE2;">${opt.label}</span>
            ${(isInserts || isGravure || !showPrices) ? '' : `<span style="font-size:12px;font-weight:500;color:${checked ? '#B08D57' : firstId ? '#aaa' : '#666'};white-space:nowrap;">${priceLabel}</span>`}
          </div>
      ...
      ${(isGravure && showPrices) ? `<div style="margin-top:.75rem;padding-top:.75rem;border-top:0.5px solid #1F2024;display:flex;justify-content:flex-end;">
        <span style="font-size:12px;font-weight:500;color:${checked ? '#B08D57' : '#666'};">${priceLabel}</span>
      </div>` : ''}
      ${isInserts ? evoRenderInsertsSubList(checked, priceLabel, showPrices) : ''}
      ${isGravure && checked ? `
      <div style="margin-top:.75rem;" onclick="event.stopPropagation()">
        <input type="text" id="evo-gravure-input" maxlength="30" value="${gravureText.replace(/"/g,'&quot;')}" placeholder="TEXTE À GRAVER (20 CARACTÈRES MAX)" oninput="evoUpdateGravureText(this.value)" style="width:100%;box-sizing:border-box;background:#141519;border:0.5px solid ${gravureError ? '#e05555' : '#333'};color:#EDEAE2;padding:8px 10px;font-size:13px;font-family:var(--font);text-transform:uppercase;letter-spacing:.03em;">
...
function evoRenderInsertsSubList(evoInsertsChecked_unused, priceLabel, showPrices) {
  ...
  return `<div style="margin-top:.75rem;padding-top:.75rem;border-top:0.5px solid #1F2024;display:flex;flex-direction:column;gap:6px;">` +
    items.map(item => {
      ...
      return `<div style="display:flex;align-items:center;gap:8px;cursor:pointer;" onclick="event.stopPropagation();evoToggleInsert('${item.id}')">
        <i class="ti ${iName}" style="font-size:13px;color:${isChecked ? '#B08D57' : '#666'};flex-shrink:0;"></i>
        <span style="font-size:12px;color:#EDEAE2;flex:1;">${item.label}${item.note ? ' — ' + item.note : ''}</span>
        <div style="width:14px;height:14px;border-radius:4px;border:0.5px solid ${isChecked ? '#B08D57' : '#444'};background:${isChecked ? '#B08D57' : 'transparent'};flex-shrink:0;display:flex;align-items:center;justify-content:center;">
          ${isChecked ? '<i class="ti ti-check" style="font-size:9px;color:#211C12;"></i>' : ''}
        </div>
      </div>`;
    }).join('') +
    (showPrices ? `<div style="display:flex;justify-content:flex-end;margin-top:4px;padding-top:6px;border-top:0.5px solid #1B1C20;">
      <span style="font-size:12px;font-weight:500;color:${anyInsertChecked ? '#B08D57' : '#666'};">${priceLabel}</span>
    </div>` : '') +
  '</div>';
}
...
function evoRenderCustomText() {
  return `<div style="margin-top:.5rem;padding:1rem;background:#141519;border:0.5px dashed #333;">
    <div style="font-size:12px;color:#888;margin-bottom:6px;">Une demande particulière non listée ci-dessus ?</div>
    <textarea id="evo-custom-text" rows="2" placeholder="Décrivez votre besoin..." oninput="evoCustomText=this.value" style="width:100%;box-sizing:border-box;background:#17181B;border:0.5px solid #333;color:#EDEAE2;padding:8px 10px;font-size:13px;font-family:var(--font);resize:vertical;line-height:1.5;">${evoCustomText}</textarea>
...
function v2DropzoneFileChange(inputId, dropzoneId) {
  ...
  dz.style.borderColor = '#B08D57';
  if (icon) { icon.className = 'ti ti-file-check'; icon.style.color = '#B08D57'; }
  if (text) text.innerHTML = file.name;
  if (text) text.style.color = '#EDEAE2';
...
function v2ChooseParcours(parcours) {
  ...
  ['standard','standard_evo','sur_mesure','hors_gamme'].forEach(p => {
    ...
    if (card) card.style.borderColor = p === parcours ? '#B08D57' : '#333';
  });
...
function calcSize() {
  ...
  if (matches.length === 1) {
    ...
    main.innerHTML = 'Taille recommandée : <span style="color:#B08D57">' + t.taille + '</span>';
    let info = 'Stature ' + t.stature_min + '–' + t.stature_max + ' cm';
    if (selle) info += ' · Entrejambe ' + t.ej_min + '–' + t.ej_max + ' cm';
    if (acro && selSize.cintre) info += ' · Cintre recommandé : <span style="color:#B08D57">' + selSize.cintre + ' mm</span>';
  ...
  if (sortedCheck[0].taille === sortedCheck[sortedCheck.length - 1].taille) {
    ...
    if (acro && selSize.cintre) infoSame += ' · Cintre recommandé : <span style="color:#B08D57">' + selSize.cintre + ' mm</span>';
    sub.innerHTML = infoSame;
    return;
  }
  main.innerHTML = 'Votre stature correspond à deux tailles : <span style="color:#B08D57">' + matches.map(t=>t.taille).join(' ou ') + '</span>';
...
function chooseUsage(usage) {
  ...
  const cintreInfo = (acroU && selSize.cintre) ? ' · Cintre recommandé : <span style="color:#B08D57">' + selSize.cintre + ' mm</span>' : '';
  document.getElementById('guide-result-main').innerHTML =
    'Taille recommandée : <span style="color:#B08D57">' + chosen.taille + '</span>' +
...
function p11UpdateStep(n) {
  ...
  for (let i=1; i<=4; i++) {
    ...
    if (sl) sl.style.color = i === n ? '#B08D57' : i < n ? '#666' : '#888';
  }
  const backBtn = document.getElementById('p11-back-btn');
  const fwdBtn  = document.getElementById('p11-fwd-btn');
  if (backBtn) backBtn.style.color = n > 1 ? '#B08D57' : '#333';
  if (fwdBtn)  fwdBtn.style.color  = (n < 4 && n !== 3) ? '#B08D57' : '#333';
  // Steps
...
function p11FileChange(inputId, dropzoneId) {
  ...
  dz.style.borderColor = '#B08D57';
  if (icon) { icon.className = 'ti ti-file-check'; icon.style.color = '#B08D57'; }
  if (text) { text.textContent = file.name; text.style.color = '#EDEAE2'; }
  ...
  container.innerHTML = opts.map(opt => {
    ...
    return `<div style="background:#17181B;border:0.5px solid ${checked ? '#B08D57' : '#1F2024'};padding:.9rem 1rem;border-radius:8px;">
      <div style="display:flex;align-items:flex-start;gap:.65rem;${isInserts ? '' : 'cursor:pointer;'}" ${isInserts ? '' : `onclick="p11EvoToggle('${opt.id}')"`}>
        ...
        <div style="flex:1;">
          <div style="display:flex;justify-content:space-between;align-items:baseline;gap:.5rem;">
            <span style="font-size:14px;font-weight:500;color:#EDEAE2;">${opt.label}</span>
      ...
      ${isGravure && checked ? `
      <div style="margin-top:.75rem;padding-top:.75rem;border-top:0.5px solid #1F2024;" onclick="event.stopPropagation()">
        <input type="text" id="p11-evo-gravure-input" maxlength="30" value="${gravureText.replace(/"/g,'&quot;')}" placeholder="TEXTE À GRAVER (20 CARACTÈRES MAX)" oninput="p11EvoUpdateGravureText(this.value)" style="width:100%;box-sizing:border-box;background:#141519;border:0.5px solid ${gravureError ? '#e05555' : '#333'};color:#EDEAE2;padding:10px;font-size:14px;font-family:inherit;text-transform:uppercase;letter-spacing:.03em;border-radius:6px;">
...
function p11EvoRenderInsertsSubList(priceLabel, showPrices) {
  ...
  if (items.length === 0) return '';
  const anyInsertChecked = items.some(i => i.avail[selModel] === 0 && evoInsertsChecked[i.id]);
  return `<div style="margin-top:.75rem;padding-top:.75rem;border-top:0.5px solid #1F2024;display:flex;flex-direction:column;gap:8px;">` +
    items.map(item => {
      const isIncluded = item.avail[selModel] === 1;
      const isChecked = evoInsertsChecked[item.id] || false;
      const iName = EVO_ICONS[item.id] || 'ti-plug';
      ...
      return `<div style="display:flex;align-items:center;gap:8px;" onclick="event.stopPropagation();p11EvoToggleInsert('${item.id}')">
        <i class="ti ${iName}" style="font-size:14px;color:${isChecked ? '#B08D57' : '#666'};flex-shrink:0;"></i>
        <span style="font-size:13px;color:#EDEAE2;flex:1;">${item.label}${item.note ? ' — ' + item.note : ''}</span>
        <div style="width:16px;height:16px;border-radius:5px;border:0.5px solid ${isChecked ? '#B08D57' : '#444'};background:${isChecked ? '#B08D57' : 'transparent'};flex-shrink:0;display:flex;align-items:center;justify-content:center;">
          ${isChecked ? '<i class="ti ti-check" style="font-size:10px;color:#211C12;"></i>' : ''}
        </div>
      </div>`;
    }).join('') +
    (showPrices ? `<div style="display:flex;justify-content:flex-end;margin-top:2px;padding-top:6px;border-top:0.5px solid #1B1C20;">
      <span style="font-size:13px;font-weight:500;color:${anyInsertChecked ? '#B08D57' : '#666'};">${priceLabel}</span>
    </div>` : '') +
  '</div>';
}
  ...
  grid.innerHTML = MODELS.map(m => {
    ...
    const presetHtml = (isExpanded && isCompletSel && hasPresets) ?
      '<div class="mc-preset-inline">' +
        ...
        '</div>' +
        '<div id="p11-preset-info" style="display:none;font-size:12px;color:#aaa;background:#1B1C20;border:0.5px solid #333;padding:10px 12px;margin-bottom:10px;line-height:1.7;">' +
...
function p11UpdateTotal() {
  ...
  if (window._activePreset && PRESETS[selModel] && PRESETS[selModel][window._activePreset]) {
    ...
    const modifText = count === 0 ? '' : '<span style="color:#B08D57;">●</span> ' + count + ' personnalisation' + (count > 1 ? 's' : '') + ' ↳ ' + window._activePreset;
...
function p11CalcSize() {
  ...
  if (matches.length === 1) {
    ...
    main.innerHTML = 'Taille recommandée : <span style="color:#B08D57">' + t.taille + '</span>';
    let info = 'Stature ' + t.stature_min + '–' + t.stature_max + ' cm';
    if (acro && selSize.cintre) info += ' · Cintre : <span style="color:#B08D57">' + selSize.cintre + ' mm</span>';
    if (warning) info += '<div style="margin-top:8px;padding:8px 10px;background:#2a1500;border-left:2px solid #e08b3a;color:#e0a370;font-size:12px;line-height:1.5;">' + warning + '</div>';
    sub.innerHTML = info;
  ...
  if (sortedCheckM[0].taille === sortedCheckM[sortedCheckM.length - 1].taille) {
    ...
    main.innerHTML = 'Taille recommandée : <span style="color:#B08D57">' + t.taille + '</span>';
    let infoSame = 'Stature ' + t.stature_min + '–' + t.stature_max + ' cm';
    if (acro && selSize.cintre) infoSame += ' · Cintre : <span style="color:#B08D57">' + selSize.cintre + ' mm</span>';
    sub.innerHTML = infoSame;
    const _nextLbl3 = document.getElementById('p11-next-label');
    if (_nextLbl3) _nextLbl3.textContent = 'Ma configuration';
    return;
  }
  main.innerHTML = 'Deux tailles : <span style="color:#B08D57">' + matches.map(t=>t.taille).join(' ou ') + '</span>';
...
function p11ChooseUsage(usage) {
  ...
  document.getElementById('p11-result-main').innerHTML =
    'Taille recommandée : <span style="color:#B08D57">' + chosen.taille + '</span> <span style="font-size:12px;color:#888">(' + (usage==='sport'?'sportif':'confort') + ')</span>';
      ...
      '<div style="display:flex;flex-direction:column;gap:5px;">' +
        tuningListP11.map(t =>
          '<div style="display:flex;justify-content:space-between;align-items:center;font-size:12px;">' +
            '<span style="color:#999;">' + t.label + ' — <span style="color:#EDEAE2;">' + t.optionName + '</span></span>' +
            '<span style="color:#B08D57;font-weight:500;flex-shrink:0;margin-left:10px;">' + (t.delta > 0 ? '+' : '') + t.delta.toLocaleString('fr-FR') + ' €</span>' +
          '</div>'
        ).join('') +
      '</div>' +
    '</div>';
...
function p11RecapBlock() {
  ...
  if (v2Parcours === 'sur_mesure') {
    ...
    return '<div style="margin-top:1rem;padding:1rem;background:#1D1F23;border:0.5px solid #333;">' +
      ...
      (msg ? '<div style="font-size:13px;color:#EDEAE2;line-height:1.6;white-space:pre-wrap;">' + msg.replace(/</g,'&lt;') + '</div>' : '<div style="font-size:13px;color:#888;font-style:italic;">Aucune description fournie.</div>') +
      (fileName ? '<div style="font-size:12px;color:#B08D57;margin-top:8px;"><i class="ti ti-paperclip"></i> ' + fileName.replace(/</g,'&lt;') + '</div>' : '') +
    '</div>' + v2EvoRecapBlockHtml('Options Évolution incluses', false);
  }
...
function p11QuickSave() {
  ...
  if (btn) {
    const orig = btn.innerHTML;
    btn.innerHTML = '<i class="ti ti-check" style="color:#B08D57;"></i>';
    setTimeout(() => { btn.innerHTML = orig; }, 1500);
  }
}
