...
async function loadConfigFromUrl() {
  ...
  try {
    ...
    if (window.innerWidth >= 768) {
      ...
      renderModels(); v2Parcours = 'standard'; p11UpdateStep(3);
    }
...
function dtGo(n) {
  ...
  // V7 — 3 étapes réelles : 1 Modèle, 2 Composants, 3 Devis. Personnalisation retirée
  // entièrement (seule la gravure survit, remontée en module dans Composants) ; Cadre
  // et Taille/Options déjà retirés en V5 (écrans fantômes, jamais vraiment utilisés).
  if (n === 3) { v2GoRecap(); return; }
  dtStep = n;
  dtRender();
  v2UpdateStepper();
}
...
function dtRender() {
  ...
  // Stepper — 3 étapes réelles (Modèle/Composants/Devis). Personnalisation retirée
  // entièrement en V7 (seule la gravure survit, en module dans Composants).
  for (let i = 1; i <= 3; i++) {
    const s = document.getElementById('dts-' + i);
    const d = document.getElementById('dts-dot-' + i);
    if (!s || !d) continue;
    s.className = 'dts-step' + (i === n ? ' active' : i < n ? ' done' : '');
    d.innerHTML = i < n ? '<i class="ti ti-check" style="font-size:9px;"></i>' : i === 3 ? '→' : String(i);
  }
  const model = MODELS.find(m => m.id === selModel);
  ...
  // Rendu spécifique V2 — seuls n=1 (Modèle) et n=2 (Composants) passent par ici ;
  // n=3 (Devis) est intercepté plus tôt par dtGo(), qui appelle v2GoRecap() directement.
  if (n === 1) dtRenderS1();
  if (n === 2) dtRenderS2();
  dtRenderRecap();
...
function dtShowSaved() {
  ...
  for (let i = 1; i <= 3; i++) {
    ...
    const d = document.getElementById('dts-dot-' + i);
    if (!s || !d) continue;
    s.className = 'dts-step' + (i === 3 ? ' active' : i < 3 ? ' done' : '');
    d.innerHTML = i < 3 ? '<i class="ti ti-check" style="font-size:9px;"></i>' : '→';
  }
...
function dtUpdateStep2Footer() {
  ...
  if (!selSize.taille) {
    ...
    zone.innerHTML =
      ...
      '<button class="dt-btn-next" id="dt-s2-btn-main" onclick="v2GoRecap()">Récapitulatif <i class="ti ti-arrow-right"></i></button>';
  }
}


// ─── CARTE "CADRE" — nouvelle en tête de l'étape Composants ──────────────────
// V7 — Gravure sur tube supérieur : seule survivante des anciens niveaux de
...
function evoToggleGravure(renderFn) {
  evoChecked['evo_gravure'] = !evoChecked['evo_gravure'];
  (renderFn === 'p11RenderPosts' ? p11RenderPosts : dtRenderPosts)();
}
function evoUpdateGravureTextRow(val, renderFn) {
  const upperVal = val.toUpperCase();
  evoGravureText = upperVal;
  // Mise à jour ciblée (pas de re-rendu complet) pour ne pas faire perdre le focus du
  // champ à chaque frappe — même logique que l'ancien evoUpdateGravureText().
  const input = document.getElementById('gravure-row-input');
  if (input && input.value !== upperVal) input.value = upperVal;
  const countEl = document.querySelector('.gravure-row-count');
  if (countEl) {
    const error = upperVal.length > 20;
    countEl.textContent = error ? 'Maximum 20 caractères, espaces compris' : (upperVal.length + ' / 20 caractères');
    countEl.classList.toggle('error', error);
  }
  if (input) input.classList.toggle('error', upperVal.length > 20);
}
...
function dtRenderPosts() {
  ...
  container.innerHTML = renderCadreCard(null, 'dtRenderPosts') + renderGravureRow('dtRenderPosts') + activePostMeta().map(p => {
...
function computeOodSurcharge() {
  ...
  // la gravure est la seule survivante, son écart de prix est simplement le sien.
  const opt = (typeof EVO_OPTIONS !== 'undefined') ? EVO_OPTIONS.find(o => o.id === 'evo_gravure') : null;
  const surcharge = (opt && evoChecked['evo_gravure']) ? opt.price : 0;
  return { surcharge, isMin: false };
}
...
function v2BackFromDevis() {
  // V7 — Personnalisation retirée, Devis (étape 3) revient directement à Composants
  // (étape 2), plus d'étape intermédiaire.
  dtGo(2);
}
...
const P11_LABELS = ['Choisir votre modèle', 'Configurer vos composants', 'Votre configuration'];
...
function p11UpdateStep(n) {
  ...
  // Dots + labels — 3 étapes réelles (Modèle/Composants/Devis). Personnalisation
  // retirée entièrement en V7 (seule la gravure survit, en module dans Composants).
  for (let i=1; i<=3; i++) {
  ...
  if (fwdBtn)  fwdBtn.style.color  = (n < 3) ? '#F5C400' : '#333';
  // Steps
  ...
  // Bandeau bas : toujours visible si un modèle est choisi (sauf étape Devis, récap
  // déjà détaillé). Seul le bouton "Suivant" est masqué sur l'étape Devis (boutons
  // d'action dédiés : "Recevoir mon devis", "Sauvegarder"...).
  ...
  const hasInlineNav = (n === 3);
  if (bar) bar.style.display = (selModel && n !== 3) ? 'block' : 'none';
  ...
  if (priceStrip) priceStrip.style.display = (selModel && n !== 3) ? 'flex' : 'none';
  const stripSave = document.getElementById('p11-strip-save');
  if (stripSave) stripSave.style.display = (n >= 2 && n !== 3 && selModel) ? 'flex' : 'none';
  ...
  // Step 3 : construire le récap final
  if (n === 3) p11RenderFinalRecap();
  window.scrollTo({ top: 0, behavior: 'smooth' });
}
...
function p11Next() {
  ...
  if (p11CurrentStep === 2) {
    ...
    p11UpdateStep(3); return; // Composants -> Devis
  }
  if (p11CurrentStep === 3) return; // navigation par boutons inline uniquement (Devis)
}
...
function p11RenderPosts() {
  ...
  container.innerHTML =
    ...
    renderCadreCard('p11-cadre-taille-select', 'p11RenderPosts') + renderGravureRow('p11RenderPosts') + activePostMeta().map(p => {
    // Poste absorbé par un combo (ex: Cintre inclus avec la potence Alanera)
    const comboLock = findComboLock(p.id);
    if (comboLock) {
      return '<div class="post-block post-block-combo-locked" data-post-id="' + p.id + '">' +
...
function dtSmartBack() {
  ...
  if (dtStep === 3) { v2BackFromDevis(); return; }
}
...
['dtGo','v2BackFromDevis'].forEach(function(fnName) {
