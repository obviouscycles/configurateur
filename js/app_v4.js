...
function computeTotals(modelId, opts) {
  ...
  activePostMeta().forEach(p => {
    ...
    // "locked" (recommandé par défaut) ne doit donc jamais faire sauter son prix,
    // contrairement au vélo complet où il est déjà compris dans basePrice.
    if (opt && (window._kitCadre || !isLocked(opt, modelId))) { price += opt.price; weight += opt.weight; }
  });
  return { price, weight };
}
...
function evoRender() {
  ...
  container.innerHTML = opts.map(opt => {
    ...
    return `<div style="background:#111;border:0.5px solid ${checked ? '#F5C400' : '#222'};padding:.9rem 1rem;border-radius:8px;transition:border-color .15s;">
      ...
      ${isGravure ? `<div style="margin-top:.75rem;padding-top:.75rem;border-top:0.5px solid #222;display:flex;align-items:center;gap:12px;">
        <img src="/configurateur/assets/evolution/votre_nom_mob.webp" alt="Exemple de gravure sur tube supérieur" style="height:112px;width:auto;aspect-ratio:3/1;object-fit:cover;border-radius:4px;flex-shrink:0;border:0.5px solid #333;">
        ${showPrices ? `<span style="font-size:12px;font-weight:500;color:${checked ? '#F5C400' : '#666'};margin-left:auto;">${priceLabel}</span>` : ''}
      </div>` : ''}
